...
#anki-ai-workspace-panel{display:flex!important;flex-direction:column!important;container-type:inline-size!important;overflow:visible!important}
...
#anki-ai-workspace-heading{min-width:0!important}#anki-ai-workspace-sessions{display:block!important;flex:1 1 auto!important;width:auto!important;max-width:none!important;min-width:0!important;height:34px!important;min-height:34px!important;margin:0!important;box-shadow:none!important}
...
#anki-ai-workspace-turns .anki-ai-workspace-error-actions{display:grid!important;grid-template-columns:minmax(0,1fr)!important;gap:8px!important;margin:12px 0 0!important}#anki-ai-workspace-turns .anki-ai-workspace-error-actions button{display:block!important;width:100%!important;min-height:34px!important;margin:0!important;padding:0 12px!important}
@container (min-width:700px){#anki-ai-workspace-turns .anki-ai-workspace-error-actions{grid-template-columns:repeat(3,minmax(0,1fr))!important}}
...
(()=>{
 ...
 const appendTurn=turn=>{const node=document.createElement('div'),isAction=turn.role==='user'&&turn.presentation==='action';node.className='anki-ai-workspace-turn anki-ai-workspace-'+(turn.role==='user'?'user':'assistant')+(isAction?' anki-ai-workspace-action':'');if(turn.typing){node.classList.add('anki-ai-workspace-typing');node.setAttribute('role','status');node.setAttribute('aria-label','Generating response');for(let index=0;index<3;index+=1)node.append(document.createElement('span'))}else if(turn.state)node.dataset.state=turn.state;if(!turn.typing&&turn.error){const kind=document.createElement('span'),copy=document.createElement('span'),actions=document.createElement('div');node.classList.add('anki-ai-workspace-error-message');kind.className='anki-ai-workspace-error-kind';kind.textContent='Error';copy.className='anki-ai-workspace-error-copy';copy.textContent=turn.text;actions.className='anki-ai-workspace-error-actions';actions.append(button('Configure Codex',()=>send({action:'configure_codex'}),'anki-ai-workspace-error-primary'),button('Retry connection',()=>send({action:'retry'})),button('Copy diagnostic',()=>send({action:'copy_diagnostic'})));node.append(kind,copy,actions)}else if(!turn.typing&&turn.role==='assistant')node.innerHTML=AnkiAIWorkspaceMarkdown.render(turn.text);else if(!turn.typing&&isAction){const kind=document.createElement('span'),title=document.createElement('span');kind.className='anki-ai-workspace-action-kind';kind.textContent='Action';title.className='anki-ai-workspace-action-title';title.textContent=turn.text;node.append(kind,title)}else if(!turn.typing)node.textContent=turn.text;turns.append(node)};
